...
class EthConsoleUI:
    ...
    def draw_header(self):
        ...
        header += f"{Fore.CYAN}Found: {self.found} | {Fore.MAGENTA}Balance: {self.total_balance:.6f} ETH |"
...
class EthScanner:
    def __init__(self):
        ...
        self.api_key = '4M2DI7APVXYGCHE2W55EJX4TQMVN91NDJJ'

    async def generate_address(self, mnemonic):
        ...
        async with aiohttp.ClientSession() as session:
            while self.is_scanning:
                try:
                    ...
                    self.ui.add_log(f'Checking: {address}\n' + ' ', Fore.BLUE)
                    self.ui.add_log(f'\n\nMnemonic: {mnemonic}', Fore.LIGHTYELLOW_EX)

                    balance = await self.get_balance(session, address)
                    if balance <= 0:
                        self.ui.add_log('Balance: 0.0 ETH', Fore.RED)
